...
function ProductMain({
  ...
  useEffect(() => {
    if (typeof document !== "undefined") {
      setTimeout(() => {
        document.dispatchEvent(new CustomEvent("bogos:product-change", {
          detail: {
            products: [{ id: product.id }],
            options: { product: true, selectedVariants: [{ product_id: product.id, id: selectedVariant?.id }] }
          }
        }));
      }, 500);
    }
    ...
    //       [{ id: product.id }], { product: true, selectedVariants: [{ product_id: product.id, id: selectedVariant?.id }] }
    //     );
    //   }, 750);
    // };
    ...
    // if (typeof document !== "undefined") {
    //   setTimeout(() => {
    //     document.dispatchEvent(new CustomEvent("bogos:bundle-init"));
    //     document.dispatchEvent(new CustomEvent("bogos:upsell-init"));
  ...
  useEffect(() => {
    ...
    if (isGift) {
      const path = window.location.pathname.split('-sca_clone_freegift')[0];
  ...
  return (
    <div className="product-main">
      <h1 style={{ width: 'fit-content', position: 'relative' }} className='fg-secomapp-product-title'>
        {title}
      </h1>
...
function ProductForm({
  ...
  return (
    <div className="product-form">
      ...
      <div className='fg-secomapp-handle-add-to-cart'></div>
